...
function bookingIndvidualEmergencyFields({ data, update }: { data: PartialDeep<JsonBookingType>["emergency"], update: any }) {
    ...
    return <>
        ...
        <TextField autoComplete="off" name="emergency-name" id="emergency-name" fullWidth sx={{ mt: 2 }} required label="Name" value={data?.name || ''} onChange={updateField('name')} />
        <TextField autoComplete="off" name="emergency-phone" id="emergnecy-phone" fullWidth  sx={{ mt: 2 }} required type="tel" label="Phone" value={data?.phone || ''} onChange={updateField('phone')} />
    </>
}
...
function bookingGroupEmergencyFields({ data, update }: { data: PartialDeep<JsonBookingType>["emergency"], update: any }) {
    ...
    return <>
        ...
        <TextField autoComplete="off" name="emergency-name" id="emergency-name" fullWidth sx={{ mt: 2 }} required label="Name" value={data?.name || ''} onChange={updateField('name')} />
        <TextField autoComplete="off" name="emergency-phone" id="emergency-phone" fullWidth sx={{ mt: 2 }} required type="tel" label="Phone" value={data?.phone || ''} onChange={updateField('phone')} />
    </>
}
