...
function bookingIndvidualEmergencyFields({ data, update }: { data: PartialDeep<JsonBookingType>["emergency"], update: any }) {
    ...
    return <>
        ...
        <TextField autoComplete="emergency name" name="emergency-name" id="emergency-name" inputProps={{'data-form-type': 'other'}} fullWidth sx={{ mt: 2 }} required label="Name" value={data?.name || ''} onChange={updateField('name')} />
        <TextField autoComplete="emergency phone" name="emergency-phone" id="emergency-phone" inputProps={{'data-form-type': 'other'}} fullWidth  sx={{ mt: 2 }} required type="tel" label="Phone" value={data?.phone || ''} onChange={updateField('phone')} />
    </>
}
