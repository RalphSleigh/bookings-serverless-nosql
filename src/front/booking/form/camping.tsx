...
function campingFields({ event, data, update }: { event: JsonEventType, data: PartialDeep<JsonBookingType>["camping"], update: any }) {
    ...
    return <>
        ...
        <TextField autoComplete="campwith" name="campwith" id="campwith" inputProps={{'data-form-type': 'other'}} fullWidth sx={{ mt: 2 }} label="Which other districts or international groups do you want to camp with?" value={data?.campWith || ''} onChange={updateField('campWith')} />
