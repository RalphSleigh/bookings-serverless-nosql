...
function CustomQestionText({ i, question, data, setArrayItem }: { i: number, question: JsonEventType["customQuestions"][0], data: any, setArrayItem: any }) {
    ...
    return (<Grid container spacing={2} mt={1} mb={1}>
        <Grid item xs>
            <TextField autoComplete="off" name={`customquestion-${i}`} id={`customquestion-${i}`} fullWidth label={question.questionLabel} value={data || ''} onChange={update} />
        </Grid>
    </Grid>)
}
...
function CustomQestionLongText({ i, question, data, setArrayItem }: { i: number, question: JsonEventType["customQuestions"][0], data: any, setArrayItem: any }) {
    ...
    return (<Grid container spacing={2} mt={1} mb={1}>
        <Grid item xs>
            <TextField autoComplete="off" name={`customquestion=${i}`} id={`customquestion-${i}`} multiline minRows={3} fullWidth label={question.questionLabel} value={data || ''} onChange={update} />
        </Grid>
    </Grid>)
}
