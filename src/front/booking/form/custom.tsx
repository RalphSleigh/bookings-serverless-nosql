...
function CustomQestionText({ i, question, data, setArrayItem }: { i: number, question: JsonEventType["customQuestions"][0], data: any, setArrayItem: any }) {
    ...
    return (<Grid container spacing={2} mt={1} mb={1}>
        <Grid item xs>
            <TextField autoComplete={`section-customquestion-${i} answer`} name={`customquestion-${i}`} id={`customquestion-${i}`} inputProps={{'data-form-type': 'other'}} fullWidth label={question.questionLabel} value={data || ''} onChange={update} />
        </Grid>
    </Grid>)
}
...
function CustomQestionLongText({ i, question, data, setArrayItem }: { i: number, question: JsonEventType["customQuestions"][0], data: any, setArrayItem: any }) {
    ...
    return (<Grid container spacing={2} mt={1} mb={1}>
        <Grid item xs>
            <TextField autoComplete={`section-customquestion-${i} answer`} name={`customquestion=${i}`} id={`customquestion-${i}`} inputProps={{'data-form-type': 'other'}} multiline minRows={3} fullWidth label={question.questionLabel} value={data || ''} onChange={update} />
        </Grid>
    </Grid>)
}
