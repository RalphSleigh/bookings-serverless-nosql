...
const BasicKP = ({ event, bookings }: { event: JsonEventType; bookings: JsonBookingWithExtraType[] }) => {
  ...
  const headerRow = KpStructure.dietOptions.map((d) => (
    <TableCell key={d}>
      <strong>{capitalize(d)}</strong>
    </TableCell>
  ));
    ...
    <Grid xs={12} p={2} item>
        ...
        <Table>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell>
                <b>{counts.reduce((a, b) => a + b.reduce((a, b) => a + b, 0), 0)}</b>
              </TableCell>
            </TableRow>
          </TableBody>
        </Table>
      ...
      <TableContainer component={Paper} sx={{ mt: 2, p: 1 }}>
        <Table>
          ...
          <TableBody>{requirementRows}</TableBody>
        </Table>
      </TableContainer>
    </Grid>
  );
...
const LargeKP = ({ event, bookings }: { event: JsonEventType; bookings: RequiredDeep<JsonBookingWithExtraType[]> }) => {
  ...
  const [attendance, setAttendance] = React.useState<"Total" | "First Three" | "Last Seven">("Total");

  for (let booking of bookings) {
    const actualParticipants = booking.participants.filter((p) => {
      if (attendance === "Total") return true;
      if (attendance === "First Three") return p.attendance.option == 0 || p.attendance.option == 1;
      if (attendance === "Last Seven") return p.attendance.option == 0 || p.attendance.option == 2;
      return false;
    });
    for (let participant of actualParticipants) {
      const ageIndex = groups.findIndex((g) => g.name == participant.ageGroup.name);
      const dietIndex = KpStructure.dietOptions.findIndex((d) => d == participant.kp!.diet.toLowerCase());
      counts[ageIndex] = counts[ageIndex] || new Array(KpStructure.dietOptions.length).fill(0);
      counts[ageIndex][dietIndex]++;
      if (
        isNonEmptyString(participant.kp.details) ||
        ...
        participant.kp.contactMe
      )
        requirements.push(participant);
    }
  }
    ...
    return (
      <TableRow key={i}>
        ...
        <TableCell>
          ...
          {isNonEmptyString(p.kp.preferences) ? (
            <>
  ...
  return (
    <Grid xs={12} p={2} item>
      <Typography variant="h6">Numbers</Typography>
      <FormControl>
        <FormLabel id="demo-radio-buttons-group-label">Attendance</FormLabel>
        <RadioGroup
          row
          aria-labelledby="demo-radio-buttons-group-label"
          value={attendance}
          onChange={(e) => setAttendance(e.target.value as "Total" | "First Three" | "Last Seven")}
          name="radio-buttons-group"
        >
          <FormControlLabel value="Total" control={<Radio />} label="Total" />
          <FormControlLabel value="First Three" control={<Radio />} label="First Three" />
          <FormControlLabel value="Last Seven" control={<Radio />} label="Last Seven" />
        </RadioGroup>
      </FormControl>
      <TableContainer component={Paper} sx={{ mt: 2, p: 1 }}>
        <Table>
...
const capitalize = (str: string = "", lowerRest = false): string => str.slice(0, 1).toUpperCase() + (lowerRest ? str.slice(1).toLowerCase() : str.slice(1));
