...
export const generateDiscordDiff: (oldBooking: BookingType, newBooking: BookingType) => string[] = (oldBooking, newBooking) => {
    ...
    const existingLatestBookingDiscord = { ...oldBooking, participants: (oldBooking.participants || [])
        .filter(p => p.basic?.name && p.basic?.dob) 
        .map(p => ({ ...p, name: p.basic?.name.trim(), basic: {...p.basic, name: p.basic.name.trim(), dob: p.basic?.dob instanceof Date && p.basic?.dob.valueOf() ? p.basic?.dob.toISOString() : p.basic?.dob}})) }
    //@ts-ignore
    const newLatestBookingDiscord = { ...newBooking, participants: (newBooking.participants || [])
        .filter(p => p.basic?.name && p.basic?.dob) 
        .map(p => ({ ...p, name: p.basic?.name.trim(), basic: {...p.basic, name: p.basic.name.trim(), dob: p.basic?.dob instanceof Date && p.basic?.dob.valueOf() ? p.basic?.dob.toISOString() : p.basic?.dob} })) }
